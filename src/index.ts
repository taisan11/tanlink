...
app.post('/api/tan/', async (c) => {
  const url = await c.req.query('url')
  if (!url) {
    return c.text('Missing url', 400)
  }
  const { key } = await shorten(c.env.TAN_LINK, url)
  return c.json({ key, url })
})

app.get('/api/links/:key/', async (c) => {
  const key = c.req.param('key')
  const res = await getUrl(c.env.TAN_LINK, key)
  if (!res) {
    return c.notFound()
  }
